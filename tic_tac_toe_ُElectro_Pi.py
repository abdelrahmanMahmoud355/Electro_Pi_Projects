...
def next_turn(row, col):
    '''
    This Function Is used to switch between two players 
    '''
    global player
...
def check_winner():
    '''
    This Function is used to check if the current player win or not
    if not switch to the 2nd player
    '''
...
def check_empty_spaces():
    '''
    This Function is used to check if there are any free button or not 
    if not its going who is the winner or no winner 
    '''
    spaces = 9
...
def start_new_game():
    '''
    This Function is used to restart the game when clicking on the restart button
    '''
    global player
...
window = Tk()  # Initiate tkinter
window.title("Tic-Tac-Toe Electro Pi")  # Name of the game

players = ["x", "o"]  # players name
player = random.choice(players)
...
window.mainloop()  # make the window alwyes open
